...
export const fetchCalendarData =
	...
	async (dispatch) => {
		const res = await csrfFetch(
			`/api/vendors/${vendorId}/vendor_calendars?duration=${duration}`
		);
		if (res.ok) {
...
export const updatePricingOptions = (vendorId, options) => async (dispatch) => {
	const res = await csrfFetch(
		`/api/vendors/${vendorId}/update_pricing_input_options`,
		{
			headers: { 'Content-Type': 'application/json' },
			method: 'PATCH',
...
const vendorsReducer = (state = {}, action) => {
	...
	switch (action.type) {
		...
		case RECEIVE_CALENDAR_DATA:
			...
			newState[calendarVendorId].calendar =
				newState[calendarVendorId].calendar ?? {};

			// Assign the calendar data
			newState[calendarVendorId].calendar.calendarData =
				calendarData.data;
			return newState;
		default:
			return state;
